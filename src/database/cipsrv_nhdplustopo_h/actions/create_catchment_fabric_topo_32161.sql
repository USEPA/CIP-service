DROP TABLE IF EXISTS cipsrv_nhdplustopo_h.catchment_32161_topo CASCADE;

SELECT topology.DropTopology('cipsrv_nhdplustopo_h_catchment_fabric_32161');

DROP SCHEMA IF EXISTS cipsrv_nhdplustopo_h_catchment_fabric_32161 CASCADE;

CREATE TABLE IF NOT EXISTS cipsrv_nhdplustopo_h.catchment_32161_topo(
...
ALTER TABLE IF EXISTS cipsrv_nhdplustopo_h.catchment_32161_topo
OWNER to cipsrv;

GRANT SELECT ON TABLE cipsrv_nhdplustopo_h.catchment_32161_topo TO PUBLIC;

CREATE INDEX IF NOT EXISTS catchment_32161_topo_i01
ON cipsrv_nhdplustopo_h.catchment_32161_topo(catchmentstatecode);

CREATE INDEX IF NOT EXISTS catchment_32161_topo_i02
ON cipsrv_nhdplustopo_h.catchment_32161_topo(nhdplusid);

SELECT topology.CreateTopology(
    toponame := 'cipsrv_nhdplustopo_h_catchment_fabric_32161'
   ,srid     := 32161
   ,prec     := 0.001
);
...
GRANT USAGE ON SCHEMA cipsrv_nhdplustopo_h_catchment_fabric_32161           TO PUBLIC;
GRANT SELECT ON TABLE cipsrv_nhdplustopo_h_catchment_fabric_32161.edge_data TO PUBLIC;
GRANT SELECT ON TABLE cipsrv_nhdplustopo_h_catchment_fabric_32161.face      TO PUBLIC;
GRANT SELECT ON TABLE cipsrv_nhdplustopo_h_catchment_fabric_32161.relation  TO PUBLIC;
GRANT SELECT ON TABLE cipsrv_nhdplustopo_h_catchment_fabric_32161.node      TO PUBLIC;

CREATE OR REPLACE PROCEDURE cipsrv_nhdplustopo_h.unravel_topology_32161(
...
BEGIN
   ...
   ,int_right_face
   FROM
   cipsrv_nhdplustopo_h_catchment_fabric_32161.edge_data a
   WHERE
   ...
   THEN
      ...
      int_objectid
      FROM
      cipsrv_nhdplustopo_h.catchment_32161_topo a
      WHERE
   ...
   LOOP
      ...
      DELETE FROM cipsrv_nhdplustopo_h.catchment_32161_topo a
      WHERE a.objectid = out_avoid_objectid[i];
    
      RAISE WARNING ' deleting relation %',int_topogeo_id;
      DELETE FROM cipsrv_nhdplustopo_h_catchment_fabric_32161.relation a
      WHERE a.topogeo_id = ary_topogeo_id[i];
      
   END LOOP;

   RAISE WARNING ' deleting edge %',p_bad_edge_id;
   PERFORM ST_RemEdgeNewFace('cipsrv_nhdplustopo_h_catchment_fabric_32161',p_bad_edge_id);
   COMMIT;
   
END 
$BODY$;

CREATE OR REPLACE PROCEDURE cipsrv_nhdplustopo_h.load_32161(
    p_state   VARCHAR
...
BEGIN
   ...
   LOOP
      ...
      DISTINCT a.objectid
      FROM
      cipsrv_epageofab_h.catchment_fabric_32161_3 a
      JOIN
      cipsrv_nhdplustopo_h_catchment_fabric_32161.edge_data b
      ON ST_INTERSECTS(a.shape,b.geom)
      WHERE
          a.catchmentstatecode = p_state
      AND NOT EXISTS (SELECT 1 FROM cipsrv_nhdplustopo_h.catchment_32161_topo c WHERE c.objectid = a.objectid)
      ...
      LOOP
         ...
         BEGIN
            INSERT INTO cipsrv_nhdplustopo_h.catchment_32161_topo(
            ...
            FROM
            cipsrv_epageofab_h.catchment_fabric_32161_3 aa
            WHERE 
         ...
         THEN
            ...
            EXECUTE 'ANALYZE cipsrv_nhdplustopo_h_catchment_fabric_32161.edge_data';
            EXECUTE 'ANALYZE cipsrv_nhdplustopo_h_catchment_fabric_32161.face';
            int_cmt := 0;
            int_anl := 0;
      ...
      THEN
         ...
         AND NOT EXISTS (SELECT 1 FROM cipsrv_nhdplustopo_h.catchment_32161_topo cc WHERE cc.objectid = aa.objectid)
...
DO $$DECLARE   
BEGIN
   CALL cipsrv_nhdplustopo_h.load_32161('PR',p_commit := 100);
   CALL cipsrv_nhdplustopo_h.load_32161('VI',p_commit := 100);
