DROP TABLE IF EXISTS cipsrv_nhdplustopo_h.catchment_3338_topo CASCADE;

SELECT topology.DropTopology('cipsrv_nhdplustopo_h_catchment_fabric_3338');

DROP SCHEMA IF EXISTS cipsrv_nhdplustopo_h_catchment_fabric_3338 CASCADE;

CREATE TABLE IF NOT EXISTS cipsrv_nhdplustopo_h.catchment_3338_topo(
...
ALTER TABLE IF EXISTS cipsrv_nhdplustopo_h.catchment_3338_topo
OWNER to cipsrv;

GRANT SELECT ON TABLE cipsrv_nhdplustopo_h.catchment_3338_topo TO PUBLIC;

CREATE INDEX IF NOT EXISTS catchment_3338_topo_i01
ON cipsrv_nhdplustopo_h.catchment_3338_topo(catchmentstatecode);
...
GRANT USAGE ON SCHEMA cipsrv_nhdplustopo_h_catchment_fabric_3338           TO PUBLIC;
GRANT SELECT ON TABLE cipsrv_nhdplustopo_h_catchment_fabric_3338.edge_data TO PUBLIC;
GRANT SELECT ON TABLE cipsrv_nhdplustopo_h_catchment_fabric_3338.face      TO PUBLIC;
GRANT SELECT ON TABLE cipsrv_nhdplustopo_h_catchment_fabric_3338.relation  TO PUBLIC;
GRANT SELECT ON TABLE cipsrv_nhdplustopo_h_catchment_fabric_3338.node      TO PUBLIC;

CREATE OR REPLACE PROCEDURE cipsrv_nhdplustopo_h.unravel_topology_3338(
...
BEGIN
   ...
   ,int_right_face
   FROM
   cipsrv_nhdplustopo_h_catchment_fabric_3338.edge_data a
   WHERE
   ...
   THEN
      ...
      int_objectid
      FROM
      cipsrv_nhdplustopo_h.catchment_3338_topo a
      WHERE
   ...
   LOOP
      ...
      DELETE FROM cipsrv_nhdplustopo_h.catchment_3338_topo a
      WHERE a.objectid = out_avoid_objectid[i];
    
      RAISE WARNING ' deleting relation %',int_topogeo_id;
      DELETE FROM cipsrv_nhdplustopo_h_catchment_fabric_3338.relation a
      WHERE a.topogeo_id = ary_topogeo_id[i];
      
   END LOOP;

   RAISE WARNING ' deleting edge %',p_bad_edge_id;
   PERFORM topology.ST_RemEdgeNewFace('cipsrv_nhdplustopo_h_catchment_fabric_3338',p_bad_edge_id);
   COMMIT;
   
END 
$BODY$;

CREATE OR REPLACE PROCEDURE cipsrv_nhdplustopo_h.load_3338(
    p_state   VARCHAR
...
BEGIN
   ...
   LOOP
      ...
      AND NOT EXISTS (SELECT 1 FROM cipsrv_nhdplustopo_h.catchment_3338_topo c WHERE c.objectid = a.objectid)
      ...
      AND (
         ...
         public.ST_INTERSECTS(
             a.shape
            ,public.ST_TRANSFORM(p_focus,3338)
         )
      )
      LIMIT int_chunk
      LOOP
         ...
         BEGIN
            INSERT INTO cipsrv_nhdplustopo_h.catchment_3338_topo(
            ...
            THEN
               ...
               THEN
                  ...
                  BEGIN
                     ...
                     ,aa.state_count
                     ,topology.toTopoGeom(
                         public.ST_UNION(aa.shape,aa.shape)
                        ,'cipsrv_nhdplustopo_h_catchment_fabric_3338'
                        ,1
                        ,0.001
                      )
         ...
         THEN
            ...
            EXECUTE 'ANALYZE cipsrv_nhdplustopo_h_catchment_fabric_3338.edge_data';
            EXECUTE 'ANALYZE cipsrv_nhdplustopo_h_catchment_fabric_3338.face';
            int_cmt := 0;
            int_anl := 0;
...
DO $$DECLARE   
BEGIN
   CALL cipsrv_nhdplustopo_h.load_3338('AK',p_commit := 100);
