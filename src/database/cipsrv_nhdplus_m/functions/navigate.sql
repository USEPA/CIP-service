...
BEGIN
   ...
   obj_start_flowline := rec.p_flowline;
   out_grid_srid := obj_start_flowline.out_grid_srid;
   
   IF NOT obj_start_flowline.innetwork
   THEN
      out_return_code    := -22;
   ...
   THEN
      ...
      IF NOT obj_stop_flowline.innetwork
      THEN
         out_return_code    := -22;
   ...
   THEN
      ...
      rec := cipsrv_nhdplus_m.get_flowline(
         ...
         ,p_permanent_identifier := p_start_permanent_identifier
         ,p_reachcode            := p_start_reachcode
         ,p_hydrosequence        := p_start_hydroseq
         ,p_measure              := p_start_measure
      );
      out_return_code       := rec.out_return_code;
      out_status_message    := rec.out_status_message;
         ...
         THEN
            ...
            SET (
               ...
               cipsrv_nhdplus_m.nav_trim_temp(
                  ...
                  ,p_maximum_distance_km  := num_maximum_distancekm
                  ,p_maximum_flowtime_day := num_maximum_flowtimeday
               ) aa
            )
            WHERE
                a.selected IS TRUE
