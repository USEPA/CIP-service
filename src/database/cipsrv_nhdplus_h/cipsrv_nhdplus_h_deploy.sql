...
BEGIN
   SELECT p.oid::regproc,pg_get_function_identity_arguments(p.oid)
   INTO a,b FROM pg_catalog.pg_proc p LEFT JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
   WHERE p.oid::regproc::text = 'cipsrv_nhdplus_h.snap_to_common_grid';
   IF b IS NOT NULL THEN EXECUTE FORMAT('DROP FUNCTION IF EXISTS %s(%s)',a,b);END IF;
EXCEPTION
   WHEN OTHERS THEN NULL;
END$$;

CREATE OR REPLACE FUNCTION cipsrv_nhdplus_h.snap_to_common_grid(
    IN  p_geometry          GEOMETRY
   ,IN  p_known_region      VARCHAR
...
WITH subselect AS (
   ...
   OR (
          s.shape && cipsrv_nhdplus_h.generic_common_mbr('3338')
      AND cipsrv_nhdplus_h.determine_grid_srid_f(s.shape) = 3338
   )
)
SELECT
...
WITH subselect AS (
   ...
   OR (
          s.shape && cipsrv_nhdplus_h.generic_common_mbr('32161')
      AND cipsrv_nhdplus_h.determine_grid_srid_f(s.shape) = 32161
   )
)
SELECT
...
ON cipsrv_nhdplus_h.catchment_32161(vpuid);

CREATE INDEX catchment_32161_11i
ON cipsrv_nhdplus_h.catchment_32161(border_status);

CREATE INDEX catchment_32161_01f
ON cipsrv_nhdplus_h.catchment_32161(SUBSTR(vpuid,1,2));
...
WITH subselect AS (
   ...
   OR (
          s.shape && cipsrv_nhdplus_h.generic_common_mbr('32702')
      AND cipsrv_nhdplus_h.determine_grid_srid_f(s.shape) = 32702
   )
)
SELECT
...
ON cipsrv_nhdplus_h.catchment_32702(border_status);
...
BEGIN
   ...
   AS (
      ...
      um
      LEFT JOIN
      cipsrv_nhdplus_h.nhdplusflow_upminordivs md
      ON
...
BEGIN
   SELECT p.oid::regproc,pg_get_function_identity_arguments(p.oid)
   INTO a,b FROM pg_catalog.pg_proc p LEFT JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
   WHERE p.oid::regproc::text = 'cipsrv_nhdplus_h.nav_ut_minordiv';
   IF b IS NOT NULL THEN EXECUTE FORMAT('DROP FUNCTION IF EXISTS %s(%s)',a,b);END IF;
END$$;

CREATE OR REPLACE FUNCTION cipsrv_nhdplus_h.nav_ut_minordiv(
    IN  p_maximum_distancekm      NUMERIC
   ,IN  p_maximum_flowtimeday     NUMERIC
...
BEGIN
   SELECT p.oid::regproc,pg_get_function_identity_arguments(p.oid)
   INTO a,b FROM pg_catalog.pg_proc p LEFT JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
   WHERE p.oid::regproc::text = 'cipsrv_nhdplus_h.nav_ut_no_minordiv';
   IF b IS NOT NULL THEN EXECUTE FORMAT('DROP FUNCTION IF EXISTS %s(%s)',a,b);END IF;
END$$;

CREATE OR REPLACE FUNCTION cipsrv_nhdplus_h.nav_ut_no_minordiv(
    IN  obj_start_flowline       cipsrv_nhdplus_h.flowline
   ,IN  num_maximum_distancekm   NUMERIC
   ,IN  num_maximum_flowtimeday  NUMERIC
) RETURNS INTEGER
...
BEGIN
   ...
   out_path_distance_km := out_path_distance_km + public.ST_DISTANCE(
       l_point
      ,sdo_temporary
   ) / 1000;
   out_nhdplusid        := out_flowlines[1].nhdplusid;
