DROP MATERIALIZED VIEW IF EXISTS cipdev_wbd.wbd_hu2_np21 CASCADE;

CREATE MATERIALIZED VIEW cipdev_wbd.wbd_hu2_np21(
    objectid
   ,tnmid
   ,metasourceid
   ,loaddate
...
FROM (
   ...
   FROM 
   cipdev_wbd.wbd_hu2_np21_5070 aa
   UNION ALL
   SELECT
    bb.tnmid
   ,bb.metasourceid
   ...
   FROM 
   cipdev_wbd.wbd_hu2_np21_3338 bb
   UNION ALL
   SELECT
    cc.tnmid
   ,cc.metasourceid
   ...
   FROM 
   cipdev_wbd.wbd_hu2_np21_26904 cc
   UNION ALL
   SELECT
    dd.tnmid
   ,dd.metasourceid
   ...
   FROM 
   cipdev_wbd.wbd_hu2_np21_32161 dd
   UNION ALL
   SELECT
    nn.tnmid
   ,gg.metasourceid
   ...
   FROM (
      ...
      ,'{' || uuid_generate_v1() || '}' AS globalid
      ,(SELECT 
        public.ST_UNION(gggg.shape) AS shape
        FROM (
           SELECT
           public.ST_TRANSFORM(a2.shape,4269) AS shape
           FROM
           cipdev_wbd.wbd_hu4_np21_32655 a2
           UNION ALL
           SELECT
           public.ST_TRANSFORM(a3.shape,4269) AS shape
           FROM
           cipdev_wbd.wbd_hu4_np21_32702 a3
        ) gggg
       ) AS shape
      FROM
...
ALTER TABLE cipdev_wbd.wbd_hu2_np21 OWNER TO cipsrv;
GRANT SELECT ON cipdev_wbd.wbd_hu2_np21 TO public;

CREATE UNIQUE INDEX IF NOT EXISTS wbd_hu2_np21_pk
ON cipdev_wbd.wbd_hu2_np21(huc2);

CREATE UNIQUE INDEX IF NOT EXISTS wbd_hu2_np21_u01
ON cipdev_wbd.wbd_hu2_np21(objectid);

CREATE UNIQUE INDEX IF NOT EXISTS wbd_hu2_np21_u02
ON cipdev_wbd.wbd_hu2_np21(globalid);

CREATE INDEX IF NOT EXISTS wbd_hu2_np21_spx
ON cipdev_wbd.wbd_hu2_np21 USING gist(shape);

ANALYZE cipdev_wbd.wbd_hu2_np21;

--VACUUM FREEZE ANALYZE cipdev_wbd.wbd_hu2_np21;
