DROP MATERIALIZED VIEW IF EXISTS cipdev_wbd.wbd_hu2_f3 CASCADE;

CREATE MATERIALIZED VIEW cipdev_wbd.wbd_hu2_f3(
    objectid
   ,tnmid
   ,metasourceid
   ,loaddate
...
FROM (
   ...
   FROM 
   cipdev_wbd.wbd_hu2_f3_26904 cc
   UNION ALL
   SELECT
    dd.tnmid
   ,dd.metasourceid
   ...
   FROM 
   cipdev_wbd.wbd_hu2_f3_32161 dd
   UNION ALL
   SELECT
    nn.tnmid
   ,gg.metasourceid
   ...
   FROM (
      ...
      ,(SELECT 
        ...
        FROM (
           ...
           FROM
           cipdev_wbd.wbd_hu4_f3_32655 a2
           UNION ALL
           SELECT
           public.ST_TRANSFORM(a3.shape,4269) AS shape
           FROM
           cipdev_wbd.wbd_hu4_f3_32702 a3
        ) gggg
       ) AS shape
      FROM
...
ALTER TABLE cipdev_wbd.wbd_hu2_f3 OWNER TO cipsrv;
GRANT SELECT ON cipdev_wbd.wbd_hu2_f3 TO public;

CREATE UNIQUE INDEX IF NOT EXISTS wbd_hu2_f3_pk
ON cipdev_wbd.wbd_hu2_f3(huc2);

CREATE UNIQUE INDEX IF NOT EXISTS wbd_hu2_f3_u01
ON cipdev_wbd.wbd_hu2_f3(objectid);

CREATE UNIQUE INDEX IF NOT EXISTS wbd_hu2_f3_u02
ON cipdev_wbd.wbd_hu2_f3(globalid);

CREATE INDEX IF NOT EXISTS wbd_hu2_f3_spx
ON cipdev_wbd.wbd_hu2_f3 USING gist(shape);

ANALYZE cipdev_wbd.wbd_hu2_f3;

--VACUUM FREEZE ANALYZE cipdev_wbd.wbd_hu2_f3;
