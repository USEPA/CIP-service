DROP MATERIALIZED VIEW IF EXISTS cipdev_wbd.wbd_hu4_f3 CASCADE;

CREATE MATERIALIZED VIEW cipdev_wbd.wbd_hu4_f3(
    objectid
   ,tnmid
   ,metasourceid
   ,loaddate
   ,areasqkm
   ,areaacres
   ,name
   ,states
   ,huc4
   ,centermass_x
   ,centermass_y
   ,globalid
   ,shape
)
AS
SELECT
 ROW_NUMBER() OVER() AS objectid
,a.tnmid
,a.metasourceid
,a.loaddate
,a.areasqkm
,a.areaacres
,a.name
,a.states
,a.huc4
,a.centermass_x
,a.centermass_y
,a.globalid
,a.shape
FROM (
   SELECT
    aa.tnmid
   ,aa.metasourceid
   ,aa.loaddate
   ,aa.areasqkm
   ,aa.areaacres
   ,aa.name
   ,aa.states
   ,aa.huc4
   ,aa.centermass_x
   ,aa.centermass_y
   ,aa.globalid
   ,public.ST_TRANSFORM(aa.shape,4269) AS shape
   FROM 
   cipdev_wbd.wbd_hu4_f3_5070 aa
   UNION ALL
   SELECT
    bb.tnmid
   ,bb.metasourceid
   ,bb.loaddate
   ,bb.areasqkm
   ,bb.areaacres
   ,bb.name
   ,bb.states
   ,bb.huc4
   ,bb.centermass_x
   ,bb.centermass_y
   ,bb.globalid
   ,public.ST_TRANSFORM(bb.shape,4269) AS shape
   FROM 
   cipdev_wbd.wbd_hu4_f3_3338 bb
   UNION ALL
   SELECT
    cc.tnmid
   ,cc.metasourceid
   ,cc.loaddate
   ,cc.areasqkm
   ,cc.areaacres
   ,cc.name
   ,cc.states
   ,cc.huc4
   ,cc.centermass_x
   ,cc.centermass_y
   ,cc.globalid
   ,public.ST_TRANSFORM(cc.shape,4269) AS shape
   FROM 
   cipdev_wbd.wbd_hu4_f3_26904 cc
   UNION ALL
   SELECT
    dd.tnmid
   ,dd.metasourceid
   ,dd.loaddate
   ,dd.areasqkm
   ,dd.areaacres
   ,dd.name
   ,dd.states
   ,dd.huc4
   ,dd.centermass_x
   ,dd.centermass_y
   ,dd.globalid
   ,public.ST_TRANSFORM(dd.shape,4269) AS shape
   FROM 
   cipdev_wbd.wbd_hu4_f3_32161 dd
   UNION ALL
   SELECT
    ee.tnmid
   ,ee.metasourceid
   ,ee.loaddate
   ,ee.areasqkm
   ,ee.areaacres
   ,ee.name
   ,ee.states
   ,ee.huc4
   ,ee.centermass_x
   ,ee.centermass_y
   ,ee.globalid
   ,public.ST_TRANSFORM(ee.shape,4269) AS shape
   FROM 
   cipdev_wbd.wbd_hu4_f3_32655 ee
   UNION ALL
   SELECT
    ff.tnmid
   ,ff.metasourceid
   ,ff.loaddate
   ,ff.areasqkm
   ,ff.areaacres
   ,ff.name
   ,ff.states
   ,ff.huc4
   ,ff.centermass_x
   ,ff.centermass_y
   ,ff.globalid
   ,public.ST_TRANSFORM(ff.shape,4269) AS shape
   FROM 
   cipdev_wbd.wbd_hu4_f3_32702 ff
   UNION ALL
   SELECT
    nn.tnmid
   ,gg.metasourceid
   ,gg.loaddate
   ,ROUND(public.ST_AREA(public.ST_TRANSFORM(gg.shape,4326)::GEOGRAPHY)::NUMERIC * 0.000001   ,4) AS areasqkm
   ,ROUND(public.ST_AREA(public.ST_TRANSFORM(gg.shape,4326)::GEOGRAPHY)::NUMERIC * 0.000247105,4) AS areaacres
   ,nn.name
   ,gg.states
   ,gg.huc4
   ,gg.centermass_x
   ,gg.centermass_y
   ,'{' || uuid_generate_v1() || '}' AS globalid
   ,gg.shape
   FROM (
      SELECT
       ggg.metasourceid
      ,ggg.loaddate
      ,ggg.states
      ,'2204' AS huc4
      ,ggg.centermass_x
      ,ggg.centermass_y
      ,ggg.globalid
      ,(SELECT public.ST_UNION(gggg.shape) FROM cipsrv_wbd.wbd_hu12_f3 gggg WHERE SUBSTR(gggg.huc12,1,4) = '2204') AS shape
      FROM
      cipsrv_wbd.wbd_hu12_f3 ggg
      WHERE
      SUBSTR(ggg.huc12,1,4) = '2204'
      LIMIT 1
   ) gg
   LEFT JOIN
   cipdev_wbd.wbd_names nn
   ON
   nn.huc = gg.huc4
) a;

ALTER TABLE cipdev_wbd.wbd_hu4_f3 OWNER TO cipsrv;
GRANT SELECT ON cipdev_wbd.wbd_hu4_f3 TO public;

CREATE UNIQUE INDEX IF NOT EXISTS wbd_hu4_f3_pk
ON cipdev_wbd.wbd_hu4_f3(huc4);

CREATE UNIQUE INDEX IF NOT EXISTS wbd_hu4_f3_u01
ON cipdev_wbd.wbd_hu4_f3(objectid);

CREATE UNIQUE INDEX IF NOT EXISTS wbd_hu4_f3_u02
ON cipdev_wbd.wbd_hu4_f3(globalid);

CREATE INDEX IF NOT EXISTS wbd_hu4_f3_f01
ON cipdev_wbd.wbd_hu4_f3(SUBSTR(huc4,1,2));

CREATE INDEX IF NOT EXISTS wbd_hu4_f3_spx
ON cipdev_wbd.wbd_hu4_f3 USING gist(shape);

ANALYZE cipdev_wbd.wbd_hu4_f3;

--VACUUM FREEZE ANALYZE cipdev_wbd.wbd_hu4_f3;


