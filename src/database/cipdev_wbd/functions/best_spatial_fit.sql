...
BEGIN
   ...
   WHERE p.oid::regproc::text = 'cipdev_wbd.best_spatial_fit';
   IF b IS NOT NULL THEN 
   EXECUTE FORMAT('DROP FUNCTION IF EXISTS %s(%s)',a,b);ELSE
   IF a IS NOT NULL THEN EXECUTE FORMAT('DROP FUNCTION IF EXISTS %s',a);END IF;END IF;
END$$;

CREATE OR REPLACE FUNCTION cipdev_wbd.best_spatial_fit(
...
BEGIN
   ...
   IF NOT cipsrv_engine.resource_exists('cipdev_wbd',str_wbd_table)
   THEN
      RAISE EXCEPTION 'wbd table not found - cipdev_wbd.%',str_wbd_table;
   ...
   FOR rec IN EXECUTE
   'SELECT a.huc12,a.shape FROM cipdev_wbd.' || str_wbd_table || ' a ' ||
