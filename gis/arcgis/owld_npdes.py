...
work_path = os.path.dirname(os.path.realpath(__file__)) + os.sep + "owld_npdes";
...
item = work_path + os.sep + container_name + os.sep + "npdes_control"
if arcpy.Exists(item):
   arcpy.Delete_management(item);
   
item = work_path + os.sep + container_name + os.sep + "npdes_attr"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_cip"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_cip_geo"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_huc12"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_huc12_geo"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_rad_a"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_rad_evt2meta"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_rad_l"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_rad_metadata"
if arcpy.Exists(item):
   arcpy.Delete_management(item);
   
item = work_path + os.sep + container_name + os.sep + "npdes_rad_p"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_rad_srccit"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_sfid"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_src2cip"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_src_a"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_src_l"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "npdes_src_p"
if arcpy.Exists(item):
   arcpy.Delete_management(item);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_control_esri";
outname   = "npdes_control";
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_attr_esri";
outname   = "npdes_attr";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);

print("adding aliases to " + outname + "...",end="",flush=True);
update_aliases(work_path + os.sep + container_name,outname,force_date=True);
print(" DONE.");

print("indexing " + outname + "...",end="",flush=True);
arcpy.management.AddIndex(target_nm,"source_joinkey"                ,"idx01");
arcpy.management.AddIndex(target_nm,"external_permit_nmbr"          ,"idx02");
arcpy.management.AddIndex(target_nm,"registry_id"                   ,"idx03");
arcpy.management.AddIndex(target_nm,"state_code"                    ,"idx04");
arcpy.management.AddIndex(target_nm,"permit_status_code"            ,"idx05");
arcpy.management.AddIndex(target_nm,"permit_type_code"              ,"idx06");
print(" DONE.");
print(outname + " export complete.");
print(" ");

###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_cip_esri";
outname   = "npdes_cip";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_cip_geo_esri";
outname   = "npdes_cip_geo";
target_nm = work_path + os.sep + container_name + os.sep + outname;
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_huc12_esri";
outname   = "npdes_huc12";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_huc12_geo_esri";
outname   = "npdes_huc12_geo";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POLYGON');
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_rad_a_esri";
outname   = "npdes_rad_a";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POLYGON');
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_rad_evt2meta_esri";
outname   = "npdes_rad_evt2meta";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_rad_l_esri";
outname   = "npdes_rad_l";
target_nm = work_path + os.sep + container_name + os.sep + outname;
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_rad_metadata_esri";
outname   = "npdes_rad_metadata";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_rad_p_esri";
outname   = "npdes_rad_p";
target_nm = work_path + os.sep + container_name + os.sep + outname;
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_rad_srccit_esri";
outname   = "npdes_rad_srccit";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_sfid_esri";
outname   = "npdes_sfid";
target_nm = work_path + os.sep + container_name + os.sep + outname;
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_src2cip_esri";
outname   = "npdes_src2cip";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_src_a_esri";
outname   = "npdes_src_a";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POLYGON');
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_src_l_esri";
outname   = "npdes_src_l";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POLYLINE');
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_npdes_src_p_esri";
outname   = "npdes_src_p";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POINT');
