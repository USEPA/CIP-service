...
work_path = os.path.dirname(os.path.realpath(__file__)) + os.sep + "owld_frspub";
...
item = work_path + os.sep + container_name + os.sep + "frspub_control"
if arcpy.Exists(item):
   arcpy.Delete_management(item);
   
item = work_path + os.sep + container_name + os.sep + "frspub_attr"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_cip"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_cip_geo"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_huc12"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_huc12_geo"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_rad_a"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_rad_evt2meta"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_rad_l"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_rad_metadata"
if arcpy.Exists(item):
   arcpy.Delete_management(item);
   
item = work_path + os.sep + container_name + os.sep + "frspub_rad_p"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_rad_srccit"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_sfid"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_src2cip"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_src_a"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_src_l"
if arcpy.Exists(item):
   arcpy.Delete_management(item);

item = work_path + os.sep + container_name + os.sep + "frspub_src_p"
if arcpy.Exists(item):
   arcpy.Delete_management(item);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_control_esri";
outname   = "frspub_control";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name,force_date=True);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_attr_esri";
outname   = "frspub_attr";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name,force_date=True);
...
arcpy.management.AddIndex(target_nm,"registry_id"                   ,"idx02");
arcpy.management.AddIndex(target_nm,"fips_code"                     ,"idx03");
arcpy.management.AddIndex(target_nm,"state_code"                    ,"idx04");
arcpy.management.AddIndex(target_nm,"us_mexico_border_ind"          ,"idx05");
arcpy.management.AddIndex(target_nm,"pgm_sys_id"                    ,"idx06");
print(" DONE.");
print(outname + " export complete.");
print(" ");

###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_cip_esri";
outname   = "frspub_cip";
target_nm = work_path + os.sep + container_name + os.sep + outname;
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_cip_geo_esri";
outname   = "frspub_cip_geo";
target_nm = work_path + os.sep + container_name + os.sep + outname;
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_huc12_esri";
outname   = "frspub_huc12";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_huc12_geo_esri";
outname   = "frspub_huc12_geo";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POLYGON');
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_rad_a_esri";
outname   = "frspub_rad_a";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POLYGON');
...
arcpy.management.AddIndex(target_nm,"geogstate"                     ,"idx13");
arcpy.management.AddIndex(target_nm,"xwalk_huc12"                   ,"idx14");
print(" DONE.");
print(outname + " export complete.");
print(" ");

###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_rad_evt2meta_esri";
outname   = "frspub_rad_evt2meta";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_rad_l_esri";
outname   = "frspub_rad_l";
target_nm = work_path + os.sep + container_name + os.sep + outname;
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_rad_metadata_esri";
outname   = "frspub_rad_metadata";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_rad_p_esri";
outname   = "frspub_rad_p";
target_nm = work_path + os.sep + container_name + os.sep + outname;
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_rad_srccit_esri";
outname   = "frspub_rad_srccit";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_sfid_esri";
outname   = "frspub_sfid";
target_nm = work_path + os.sep + container_name + os.sep + outname;
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_src2cip_esri";
outname   = "frspub_src2cip";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = tbexporter(source,outname,work_path,container_name);
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_src_a_esri";
outname   = "frspub_src_a";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POLYGON');
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_src_l_esri";
outname   = "frspub_src_l";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POLYLINE');
...
###############################################################################
source    = source_conn + r"/cipsrv_gis.owld_frspub_src_p_esri";
outname   = "frspub_src_p";
target_nm = work_path + os.sep + container_name + os.sep + outname;
ret       = fcexporter(source,outname,work_path,container_name,'POINT');
